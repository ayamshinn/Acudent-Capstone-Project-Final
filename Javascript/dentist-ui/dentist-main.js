...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const initialPage = location.hash ? location.hash.substring(1) : '../../HTML/dentist-ui/dentist-subfolder/dentist-dashboard.html';
    ...
    window.addEventListener("popstate", (event) => {
      if (event.state && event.state.pageUrl) {
        ...
        loadPage('../../HTML/dentist-ui/dentist-subfolder/dentist-dashboard.html', false);
      }
    });
});
...
function initPageScript(pageUrl) {
  ...
  if (pageUrl.includes('dentist-patient-records-detailed.html')) {
